...
export function AskUserQuestionCard({
  ...
  return (
    <div className="rounded-xl border border-primary/30 bg-primary/5 p-3">
      ...
      <p className="mt-1 text-sm font-medium text-text">{input.question}</p>
        ...
        {input.options.map((opt) => {
          ...
          return (
            <button
              ...
              className={`group flex flex-col items-start gap-0.5 rounded-lg border px-3 py-2 text-left text-sm transition-colors ${
                isChosen
                  ? "border-primary bg-primary text-white"
                  : isAnswered
                    ? "border-border bg-surface-alt/40 text-text-secondary"
                    : "border-border bg-background hover:border-primary/40 hover:bg-primary/10 text-text"
              }`}
            >
              ...
              {opt.description && (
                <span
                  className={`text-xs ${
                    isChosen ? "text-white/80" : "text-text-secondary"
                  }`}
                >
                  {opt.description}
                </span>
