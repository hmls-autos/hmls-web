...
export function createCalcomTools(apiKey: string, eventTypeId: string) {
  ...
  const getAvailabilityTool = {
    ...
    execute: async (
      ...
      const data = await calcomRequest<{ slots?: unknown[] }>(
        `/availability?eventTypeId=${eventTypeId}&startTime=${start}&endTime=${end}`,
      );
  ...
  return [getAvailabilityTool, createBookingTool];
}
